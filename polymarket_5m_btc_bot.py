...
async def init_client():
    ...
    try:
        # Updated initialization for latest py-clob-client (MetaMask / EOA wallet)
        client = ClobClient(
            host="https://clob.polymarket.com",
            key=PRIVATE_KEY,           # Your MetaMask private key
            chain_id=POLYGON,
            signature_type=2,          # 2 = Browser / Gnosis Safe style (MetaMask)
            funder=WALLET_ADDRESS      # Your wallet address that holds the funds
        )
        
        # Derive API credentials (required for trading)
        client.set_api_creds(client.create_or_derive_api_creds())
        
        print("✅ LIVE client connected to Polymarket (MetaMask wallet)")
    except Exception as e:
        print(f"❌ Client init failed: {e}")
        raise


# ================== MARKET DISCOVERY (your original) ==================
