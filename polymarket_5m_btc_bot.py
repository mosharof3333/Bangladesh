...
def print_dashboard(event: str):
    ...
    print(f"Last: {'+' if last_trade_pnl >= 0 else ''}{last_trade_pnl:.2f} | Total: {'+' if total_pnl >= 0 else ''}{total_pnl:.2f}")
...
# Improved orderbook fetch with bid + ask
def get_orderbook_info_sync(token_id: str):
    if DEMO_MODE:
        import random
        p = round(0.45 + random.random() * 0.50, 4)
        return Decimal(str(p)), Decimal(str(p))  # fake ask, mid
    ...
    while True:
        ...
        if markets:
            ...
            if time.time() - last_print > 2:
                ...
                print(f"[{time.strftime('%H:%M:%S')}] UP Ask: {up_ask:.4f} (Mid \~{up_mid:.4f})  |  DOWN Ask: {down_ask:.4f} (Mid \~{down_mid:.4f})")

                if up_ask >= ENTRY_THRESHOLD or down_ask >= ENTRY_THRESHOLD:
                    print("🔥 MOMENTUM SIGNAL (ask ≥ 0.65) DETECTED!")

                last_print = time.time()

            if active_entry_order_id is None and position_token_id is None:
                up_ask, _ = await get_orderbook_info(markets["up_token_id"])
                down_ask, _ = await get_orderbook_info(markets["down_token_id"])
...
# (keep the rest of the file: monitor_position, close_position, main — same as previous version)
